...
namespace COPPlatform.Controllers
{
    ...
    public class AssessmentResponseController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAssignedAssessments()
        {
            ...
            var result = await _responseService.GetAssignedAssessments(userId.GetValueOrDefault(), userRole);
            return Ok(result);
        }


        [HttpGet]
        [Route("getAssignedInvitations")]
        [Authorize]
        public async Task<IActionResult> GetAssignedInvitations()
        {
            var userId = GetUserIdFromClaims();
            if (userId == null)
                return Unauthorized("User ID not found in token.");
            ...
            var result = await _responseService.GetAssignedInvitations(userId.GetValueOrDefault(), userRole);
            return Ok(result);
        }
