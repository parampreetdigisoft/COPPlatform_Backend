        ...
        public async Task<ResultResponseDto<string>> SendMailForEditAssessment(SendRequestMailToUpdateCity request)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (assessment != null)
                    {
                        var city = _context.Cities.FirstOrDefault(x => x.CityID == assessment.UserAssessmentMapping.CityID);

                        var url = string.Empty;
                        if (mailToUser.Role == UserRole.Admin)
                        {
                            url = $"admin/assesment/2/{assessment.UserAssessmentMapping.CityID}";
                        }
                        else
                        {
                            url = $"analyst/evaluator-response/{request.UserID}/{assessment.UserAssessmentMapping.CityID}";
                        }
